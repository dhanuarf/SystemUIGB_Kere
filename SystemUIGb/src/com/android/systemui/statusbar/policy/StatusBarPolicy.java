/*
 * Coyright (C) 2008 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the ;@Licensep);
 ...
 * bar at boot time.  It goes through the no  rmal API for icons, even though it probably
...
*/
   ...
   public StatusBarPolicy(Context context) {
       ...
       // boolean isWimaxEnabled = mContext.getResources().getBoolean(
        ...
        // Alarm clock
        mService.setIcon("alarm_clock", R.drawable.stat_sys_alarm, 0);
        mService.setIconVisibility("alarm_clock", false);

        // Sync state
        mService.setIcon("sync_active", com.android.internal.R.drawable.stat_notify_sync, 0);
...
//        filter.addAction(WifiManager.RSSI_CHANGED_ACTION);
	...
	View.OnClickListener lowBatt=new View.OnClickListener(){

		@Override
		public void onClick(View p1)
		{
			if(p1.getId()==R.id.btn_right)
				closeHeadsUpLowBattery();
							// TODO: Implement this method
		}
	};

	private LinearLayout lowBatteryHeadsUp;
	private boolean isLowBatteryShowed;
	private void addHeadsUpLowBattery(){
		// low battery heads up notif -----
		// tutup sit ben ra numpuk
		closeHeadsUpLowBattery();
		final View v= View.inflate(mContext, R.layout.battery_low_heads_up,null);
		lowBatteryHeadsUp = new LinearLayout(mContext);
		// get current color
		lowBatteryHeadsUp.setBackgroundColor(ColorBarTint.getCurrentBarColor());
		final boolean isBright=ColorUtils.isBrightColor(ColorBarTint.getCurrentBarColor());
		
		final ImageView ic=(ImageView)v.findViewById(R.id.ic_connect_charger);
		final TextView title=(TextView)v.findViewById(R.id.warning_text);
		final TextView detail=(TextView)v.findViewById(R.id.detailed_warning);
		final Button btnRight=(Button)v.findViewById(R.id.btn_right);
		
		ic.setColorFilter(isBright? 0xff222222 : 0xffffffff);
		title.setTextColor(isBright? 0xff111111 : 0xffffffff);
		detail.setTextColor(isBright? 0xff333333 : 0xffeeeeee);
		btnRight.setTextColor(isBright? 0xff222222 : 0xffeeeeee);
		
		v.findViewById(R.id.btn_left).setOnClickListener(lowBatt);
		btnRight.setOnClickListener(lowBatt);
		title.setText(R.string.battery_low_title);
		final CharSequence levelText = mContext.getString(R.string.battery_low_subtitle) +"\n"+ mContext.getString(R.string.battery_low_percent_format, mBatteryLevel);
		detail.setText(levelText);
		lowBatteryHeadsUp.setOnClickListener(lowBatt);
		lowBatteryHeadsUp.addView(v);
		// --------
	...
	//	| WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN
		...
		lowBatteryHeadsUp.setMinimumHeight(60);
	//	lp.height = 
		WindowManagerImpl.getDefault().addView(lowBatteryHeadsUp, lp);
		isLowBatteryShowed=true;
	}
	
	private void closeHeadsUpLowBattery(){
		if(lowBatteryHeadsUp!=null && isLowBatteryShowed){
			WindowManagerImpl.getDefault().removeView(lowBatteryHeadsUp);
    ...
    private final void updateBattery(Intent intent) {
        ...
        mBatteryLevel = level;
	//	Log.d("BATRE", "level: "+ mBatteryLevel);
	...
	private void addFullChargeNotification() {
		CharSequence r1_CharSequence = "Batre Full coy!";
		Notification r2_Notification = new Notification(R.drawable.ic_notification_overlay, r1_CharSequence, 0);
		r2_Notification.flags |= 2;
		r2_Notification.setLatestEventInfo(mContext, r1_CharSequence ,"Cabut colokan ben aman!", PendingIntent.getActivity(mContext, 0, new Intent(), 0));
		((NotificationManager) mContext.getSystemService("notification")).notify(117506048, r2_Notification);
	//	playTone();
		turnOnScreenWithForce();
	}
	private void removeFullChargeNotification() {
		((NotificationManager) mContext.getSystemService("notification")).cancel(117506048);
	}
	private void turnOnScreenWithForce() {
		IPowerManager tPMS = IPowerManager.Stub.asInterface(ServiceManager.getService("power"));
		try {
			tPMS.userActivityWithForce(SystemClock.uptimeMillis(), false, true);
